...
- **Kafka-Based Real-Time Processing**:
  ...
  - Publishes processed insights to a Kafka output topic every minute for the last minute.
...
- **Scalable and Fault Tolerant**:
  ...
  - GitHub Actions and CI are utilized to ensure that production can't break.
...
3. **`internal/kafka`**:
   ...
   - **Producer**: A generic implementation for publishing messages to topics (e.g., `processed-login`) with reliable delivery using `IBM/Sarama`.
...
1. Build and run the Docker containers:
  ...
  ```
1. Verify Kafka topics: (`user-login` and `processed-login`)
  ```bash
  docker exec -it golang_kafka_testing-kafka-1 kafka-topics --bootstrap-server kafka:9092 --list
  ```
1. Watch the logging for the User Login Insights on a minute-by-minute basis (will not populate until the container has been running 1+ minutes):
  ```bash
  docker-compose logs -f my-golang-consumer
  ```
1. Kill the Docker containers:
  ```bash
  docker-compose down
  ```
